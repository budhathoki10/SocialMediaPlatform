...
- **Always push to origin after committing.** Every local commit gets pushed to the matching `origin` branch right after it's made — don't leave commits sitting local-only, and don't ask for confirmation to push first. This is standing authorization for `git push` in this repo.
