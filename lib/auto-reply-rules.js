...
import { canSendAnotherAutoReply, getCurrentPlan, planAllowsAutoReply } from "./entitlements.js";
import { AutoReplyRule, AutoReplySettings, InstagramDraft } from "./models.js";
import { getKathmanduDate, getStartOfKathmanduDay } from "./models.js";
...
// True only when the message IS the greeting — nothing substantive follows
// it (e.g. "hey", "hello!", "yo 👋"). A greeting word merely opening a real
// question ("heyyy who is the GOAT of football") must NOT count as pure —
// GREETING_PATTERN only anchors to the start of the string, so without this
// check every message that happens to open with "hi"/"hey"/"hello" would be
// classified as a bare greeting and get the canned "Thanks for reaching
// out" template, regardless of what the sender actually asked.
function isPureGreeting(message) {
  if (!GREETING_PATTERN.test(message)) return false;
  const remainder = message
    .replace(GREETING_PATTERN, "")
...
function categorize(message, semanticAnalysisEnabled) {
  ...
  // Small talk is checked before "pure greeting" so a message like "hello
  // bro how are you" — which opens with a greeting word but is really
  // asking something — gets recognized as small talk rather than being
  // truncated down to just the opening word.
...
Answer immediately with that JSON object and nothing else. Do not narrate your reasoning, do not write a thinking process, do not restate these instructions — reasoning text spends the token budget before the JSON is ever emitted, and the response is then discarded as unparseable.`;
}
...
function parseModelJson(raw) {
  ...
  try {
    ...
    // Reasoning-tuned models sometimes prepend a thinking preamble despite
    // being told not to. The payload we want is the last balanced {...} block,
    // so recover it rather than throwing the whole draft away.
...
async function requestAiDraft({ settings, message, senderName, isFirstContact, history }) {
  ...
  try {
    raw = await generateNvidiaText(userPrompt, { system: systemPrompt, temperature: 0.4, json: true });
  } catch (error) {
...
async function generateAiDraft({ settings, message, senderName, isFirstContact, history = [], allowRegenerate = false }) {
  ...
  // The regenerate-once pass costs a second full model call (~15s) on exactly
  // the drafts that are already headed for human review, which is where the
  // queue drain budget gets spent. Off by default; opt in per call.
  if (firstAttempt.needsHuman && allowRegenerate) {
 ...
 *
 * Plan is also checked here rather than only at the settings-save layer —
 * `AutoReplySettings.enabled` can go stale (true) after a downgrade, since
 * downgrading only affects the Subscription record, not every user's
 * settings document. Like quota below, a plan that doesn't allow auto-reply
 * never blocks draft generation itself — it only downgrades a "ready" draft
 * to needs_review so it can't auto-send.
 */
export async function buildRuleBasedDraft({ userId, message, senderId, senderName, source }) {
  ...
  const planAllowed = planAllowsAutoReply(plan);

  // Belt-and-suspenders alongside the tone downgrade below: force the agent
  // toggle off in the settings object handed back to the caller so a stale
  // `enabled: true` left over from a downgrade (see doc comment above) can
  // never read as "on" for a plan that doesn't allow auto-send.
  const effectiveSettings = planAllowed ? settings : { ...settings, enabled: false };

  const result = await computeDraft({ settings: effectiveSettings, message, senderId, senderName, userId, source });

  // Plan and quota only ever hold back auto-send — the draft itself is
  // always composed for real (AI/rules/keyword template) so the human
  // reviewer sees an actual suggested reply, not a canned placeholder. A
  // draft that would've auto-sent ("ready") gets downgraded to
  // needs_review instead; anything already needs_review/blocked for its
  // own reason (spam, hours, contact filters, etc.) is untouched.
  if (!planAllowed && result.tone === "ready") {
    return {
      ...result,
      tone: "needs_review",
      reason: `plan_restricted — draft generated but held for manual review: ${result.reason}`,
      settings: effectiveSettings,
    };
  }

  if (result.tone === "ready") {
    const quota = await canSendAnotherAutoReply(userId, plan);

    if (!quota.allowed) {
      return {
        ...result,
        tone: "needs_review",
        reason: `monthly_auto_reply_quota_reached (${quota.limit}) — draft generated but held for manual review: ${result.reason}`,
        settings: effectiveSettings,
      };
    }
  }
...
async function computeDraft({ settings, message, senderId, senderName, userId, source }) {
  ...
  // Off by default: no per-contact daily cap, so AI keeps replying to the
  // same person as needed, bounded only by the plan's overall monthly quota
  // (checked earlier in buildRuleBasedDraft). Only enforced when the user
  // explicitly turns this on.
  const maxRepliesEnabled = settings.response_style?.max_replies_per_contact_enabled ?? false;
  const maxReplies = settings.response_style?.max_replies_per_contact ?? 1;

  if (maxRepliesEnabled && senderId) {
    const sentCount = await InstagramDraft.countDocuments({
      user_id: userId,
      sender_id: senderId,
      status: "sent",
      sent_at: { $gte: getStartOfKathmanduDay() },
    });

    if (sentCount >= maxReplies) {
      return {
        tone: "needs_review",
        draftReply: `This contact has already reached today's auto-reply limit (${maxReplies} per day) — it resets at midnight, please respond manually.`,
        confidence: null,
        reason: `reply limit reached (${sentCount}/${maxReplies} today, resets at midnight)`,
      };
    }
  }
