...
import { canSendAnotherAutoReply, planAllowsAutoReply } from "./entitlements.js";
import { AutoReplyRule, AutoReplySettings, InstagramDraft, User } from "./models.js";
 ...
 * (true) after a downgrade, since downgrading only updates `User.plan`, not
 * every user's settings document. This is the runtime gate that actually
 * stops replies going out once a plan no longer allows it.
 */
export async function buildRuleBasedDraft({ userId, message, senderId, senderName, source }) {
  ...
  if (!quota.allowed) {
    return {
      tone: "needs_review",
      draftReply: `This month's auto-reply limit (${quota.limit}) has been reached — please respond manually, or upgrade your plan for more.`,
      confidence: null,
      reason: "monthly_auto_reply_quota_reached",
      settings,
    };
  }
