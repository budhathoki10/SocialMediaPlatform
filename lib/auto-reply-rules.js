...
const EMOJI_GUIDANCE = {
  ...
  minimal: "at most one, try to fit at least one but only if it fits naturally",
...
export async function buildRuleBasedDraft({ userId, message, senderId, senderName }) {
  ...
  const result = await computeDraft({ settings, message, senderId, senderName, userId });

  return { ...result, settings };
}

async function computeDraft({ settings, message, senderId, senderName, userId }) {
  ...
  if (category !== "unclear") {
    return {
      tone: "ready",
      draftReply: composeReply(settings, senderName, category),
