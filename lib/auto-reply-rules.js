 ...
 *
 * It DOES depend on the user's current plan, checked here rather than only
 * at the settings-save layer — `AutoReplySettings.enabled` can go stale
 * (true) after a downgrade, since downgrading only affects the Subscription
 * record, not every user's settings document. This is the runtime gate that
 * actually stops replies going out once a plan no longer allows it.
 */
export async function buildRuleBasedDraft({ userId, message, senderId, senderName, source }) {
  ...
  if (!planAllowsAutoReply(plan)) {
    return {
      tone: "blocked",
      draftReply: "Auto-reply is available on the Pro and Unlimited plans — upgrade to enable automatic replies.",
      confidence: null,
  ...
  // Quota only ever holds back sending — the draft itself is always
  // composed for real (AI/rules/keyword template) so the human reviewer
  // sees an actual suggested reply, not a canned placeholder. A draft that
  // would've auto-sent ("ready") gets downgraded to needs_review instead;
  // anything already needs_review/blocked for its own reason (spam, hours,
  // contact filters, etc.) is untouched.
