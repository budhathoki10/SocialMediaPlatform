...
// The single query fragment every "does this user currently have paid
// access" check shares. There is deliberately no cron job that periodically
// downgrades a subscription once its period ends — activity is computed
// live, the same way the Freemius SDK's own EntitlementService.getActives()
// decides it: a stored expiration date compared against `now` at read time.
// `current_period_end: null` is kept "active" (used for e.g. free_trial
// rows that never got a period end), everything else must not have expired
// yet. Webhook-driven `status` still has to be in ACTIVE_PLAN_STATUSES —
// this only adds the time bound on top, it doesn't replace that check.
export function activeSubscriptionFilter(userId) {
  return {
    user_id: userId,
...
// A user with no matching Subscription (never purchased, last one
// cancelled/expired, or its current_period_end has simply passed) is free —
// there's no "free" Subscription document, absence of an active one IS the
// free plan.
