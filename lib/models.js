...
const autoReplySettingsSchema = new Schema(
  {
    ...
    response_style: {
      ...
      sign_off_style: { type: String, default: "team_name" },
      custom_sign_off: { type: String, default: "AutoPilot Support Team" },
...
const subscriptionSchema = new Schema(
  {
    ...
    status: {
      ...
      // cancellation. "expired" = it simply ran past current_period_end
      // with no renewal (our own time-based cutoff in
      // expireOverdueSubscriptions, or Freemius's license.expired) — nobody
      // actively cancelled anything, the plan just ran out. Keeping these
      // separate matters for support/billing history, even though both are
      // treated identically for access purposes (see ACTIVE_PLAN_STATUSES
      // in lib/entitlements.js — neither grants access).
      enum: [
        "active",
        "trialing",
        "past_due",
    ...
    // Set only for status: "expired" (ran past current_period_end with no
    // renewal — see expireOverdueSubscriptions in lib/subscriptions.js).
