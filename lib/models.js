...
const instagramDraftSchema = new Schema(
  {
    ...
    // The webhook writes the row before the model has drafted anything, so the
    // Meta delivery can be acknowledged in under a second. The queued job flips
    // this to "done" (or "failed") once the reply is composed. Rows written
    // before this field existed have no value and are treated as "done".
    ai_status: { type: String, enum: ["pending", "done", "failed"], default: "done", index: true },
