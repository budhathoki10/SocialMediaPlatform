...
function getToneFromMessage(message) {
  ...
  return {
    tone: "needs_review",
    draftReply: "I couldn't generate a confident reply for this message — please review and respond manually.",
  };
}
...
export async function upsertInstagramDraft({
  ...
  // When auto-reply is enabled for this user, this checks their configured
  // rules (platform access, keyword exclusions, contact filters, business
  // hours, reply limits) and drafts accordingly. Returns null when
  // automation is off/unconfigured, so it falls back to the plain
  // keyword-based draft that always existed.
  const ruleBasedDraft = await buildRuleBasedDraft({
    userId,
  ...
  if (ruleBasedDraft) {
    console.log("Auto-reply rule result:", { userId: String(userId), senderId, reason: ruleBasedDraft.reason });
  }

  const generatedDraft = ruleBasedDraft || getToneFromMessage(originalMessage);
