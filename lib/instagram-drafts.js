...
export async function recordInstagramInboundEvent({
  ...
  // A DM webhook only carries the sender's numeric id, and the real profile is
  // fetched later by the queued job. Seed from the last draft we already hold
  // for this sender so the inbox shows their name and avatar immediately
  // instead of a bare id — it's an indexed local read, no network call. The
  // job still refreshes it afterwards.
  const knownProfile = senderId
    ? await InstagramDraft.findOne({ user_id: userId, sender_id: senderId, sender_name: { $ne: null } })
        .select("sender_name sender_username sender_profile_picture_url")
        .sort({ created_at: -1 })
        .lean()
    : null;

  // Only carry over an avatar we host ourselves. Meta's own profile_pic links
  // are short-lived and 404 once they expire, so copying one onto a new draft
  // would just spread a dead image; leave it null and let the job fetch a
  // fresh one and mirror it to Cloudinary.
  const seededProfilePicture = String(knownProfile?.sender_profile_picture_url || "").includes("res.cloudinary.com")
    ? knownProfile.sender_profile_picture_url
    : null;
  ...
  const draft = await InstagramDraft.findOneAndUpdate(
    { user_id: userId, external_id: externalId },
    {
      $setOnInsert: {
        ...
        sender_name: knownProfile?.sender_name || null,
        sender_username: knownProfile?.sender_username || normalizedSenderUsername,
        sender_profile_picture_url: seededProfilePicture,
...
export async function finalizeInstagramDraftReply({ userId, draftId, senderProfile = null }) {
  ...
  try {
    ...
    // Deliberately leave ai_status as "pending" and rethrow: marking it failed
    // here would make this function early-return on the very next attempt, so
    // BullMQ's retries — and the stuck-draft sweep — would both become no-ops
    // and the row would sit on the placeholder forever.
