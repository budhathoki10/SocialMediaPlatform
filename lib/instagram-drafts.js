 ...
 *
 * Returns `{ draftId, isNew }`; `isNew` is false for a redelivery of an event
 * that was already recorded, so the caller can skip re-queueing the work.
 */
export async function recordInstagramInboundEvent({
  ...
  const existing = await InstagramDraft.findOne({ user_id: userId, external_id: externalId })
    .select("_id")
    .lean();
  ...
  return { draftId: draft._id, isNew: !existing };
}
