...
async function sendInstagramPrivateReply({ account, commentId, message }) {
  if (!account.page_id) {
    console.warn("Skipping Instagram private reply — connected account has no Facebook Page ID.");
    return false;
  }

  try {
    const response = await fetch(`https://graph.facebook.com/v25.0/${account.page_id}/messages`, {
      method: "POST",
      headers: {
