...
import { AlertCircle, Check, CheckCircle2, Minus, Pencil, Plus, X as XIcon } from "lucide-react";
...
const textareaClassName =
  "w-full resize-none rounded-control border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 outline-none transition focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/10";
...
export default function AutoReplySettingsPanel({
  ...
  initialRules: AutoReplyKeywordRule[];
  ...
  async function addRule(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Unable to add keyword reply.");
      }
  ...
  function startEditRule(rule: AutoReplyKeywordRule) {
    setRuleError("");
  ...
  async function saveEditRule(id: string) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Unable to update keyword reply.");
      }

      setRules((prev) => prev.map((rule) => (rule.id === id ? data.rule : rule)));
      setEditingRuleId(null);
    } catch (error) {
      setRuleError(error instanceof Error ? error.message : "Unable to update keyword reply.");
    } finally {
  ...
  async function deleteRule(id: string) {
    ...
    try {
      const response = await fetch(`/api/auto-reply/rules/${id}`, { method: "DELETE" });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.error || "Unable to delete keyword reply.");
      }

      setRules((prev) => prev.filter((rule) => rule.id !== id));
      if (editingRuleId === id) setEditingRuleId(null);
    } catch (error) {
      setRuleError(error instanceof Error ? error.message : "Unable to delete keyword reply.");
    } finally {
        ...
        <div className="contents lg:block lg:space-y-5">
          ...
          <Card className="order-2 lg:order-none">
            ...
            <div className="space-y-3 pt-4">
              {rules.length === 0 ? (
                <p className="text-sm text-slate-400">No keyword replies configured yet.</p>
              ) : (
                    ...
                    {rules.map((rule) =>
                      editingRuleId === rule.id ? (
                        <motion.div
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="space-y-2 rounded-control border border-primary/30 bg-primary/5 p-3"
                        >
                          <input
                            value={editRuleKeyword}
                            onChange={(event) => setEditRuleKeyword(event.target.value)}
                            placeholder="Keyword"
                          ...
                          <textarea
                            value={editRuleReply}
                            onChange={(event) => setEditRuleReply(event.target.value)}
                            placeholder="Message to send"
                            aria-label="Edit reply message"
                            rows={2}
                          ...
                          <div className="flex justify-end gap-2">
                            <PressableButton
                              ...
                              className="h-8 rounded-control px-3 text-xs font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 disabled:opacity-50"
                            >
                              Cancel
                            </PressableButton>
                            <PressableButton
                              ...
                              disabled={editRuleSaving || !editRuleKeyword.trim() || !editRuleReply.trim()}
                              className="h-8 rounded-control bg-primary px-3 text-xs font-bold text-white transition hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-60"
                            >
                        ...
                        <motion.div
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="flex items-start justify-between gap-3 rounded-control border border-slate-200 p-3"
                        >
                          <div className="min-w-0">
                          ...
                          <div className="flex shrink-0 items-center gap-1">
                            <button
                              ...
                              aria-label={`Edit keyword reply for ${rule.keyword}`}
                              className="grid h-7 w-7 place-items-center rounded-control text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
                            >
                            ...
                            <button
                              ...
                              aria-label={`Delete keyword reply for ${rule.keyword}`}
                              className="grid h-7 w-7 place-items-center rounded-control text-slate-400 transition hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                            >
              ...
              <form onSubmit={addRule} className="space-y-2 rounded-control border border-dashed border-slate-300 p-3">
                <input
                ...
                <textarea
                  value={newRuleReply}
                  onChange={(event) => setNewRuleReply(event.target.value)}
                  placeholder="Exact message to send when this keyword matches"
                  aria-label="New reply message"
                  rows={2}
                  className={textareaClassName}
                />
                <div>
                ...
                <div className="flex justify-end">
                  <PressableButton
                    type="submit"
                    disabled={addingRule || !newRuleKeyword.trim() || !newRuleReply.trim()}
                    className="inline-flex h-9 items-center gap-1.5 rounded-control bg-primary px-4 text-xs font-bold text-white shadow-card transition hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <Plus className="h-3.5 w-3.5" />
                    {addingRule ? "Adding…" : "Add keyword reply"}
                  </PressableButton>
                </div>
              </form>
            </div>
